...
usage() {
  echo "Usage:"
  echo "  $0 --adc firebase-project-id [--dry-run]"
  echo "  $0 /absolute/path/to/firebase-service-account.json firebase-project-id [--dry-run]"
  exit 1
}
...
if [ "$1" = "--adc" ]; then
  ...
  docker run --rm \
    ...
    node:22 \
    sh -lc "npm install && node ./scripts/migrate-dog-doc-ids.mjs --project-id ${PROJECT_ID} $*"
